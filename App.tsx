...
const ShareIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
    <path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.287.696.345 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z" />
  </svg>
);
...
export default function App() {
  ...
  const [lists, setLists] = useState<ShoppingList[]>([
    {
      ...
      sharedWith: ['user_456'],
      items: [
        { id: '1', name: 'Milk', groupId: GroupId.DAIRY, isChecked: false, addedBy: 'user_123', timestamp: Date.now() },
        { id: '2', name: 'Bread', groupId: GroupId.BAKERY, isChecked: true, addedBy: 'user_123', timestamp: Date.now() }
      ]
    }
  ]);
  ...
  const handleAddItem = async () => {
    ...
    notificationTimerRef.current = window.setTimeout(() => {
      ...
      console.log("Dispatching notification for items:", pendingItemsRef.current);
  ...
  // Group Sorting Logic
  const sortedGroups = [...DEFAULT_GROUPS].sort((a, b) => {
  ...
  // Group Items Logic
  const groupedItems = activeList ? sortedGroups.map(group => {
    const itemsInGroup = activeList.items.filter(i => i.groupId === group.id);
    return {
      group,
      items: itemsInGroup
    };
  ...
  const otherItems = activeList ? activeList.items.filter(i => !sortedGroups.find(g => g.id === i.groupId)) : [];
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-600 text-white p-6">
        ...
        <div className="mt-8 flex gap-4">
          <button onClick={() => setLang(Language.HE)} className={`font-bold ${lang === Language.HE ? 'underline' : 'opacity-50'}`}>עברית</button>
          <button onClick={() => setLang(Language.EN)} className={`font-bold ${lang === Language.EN ? 'underline' : 'opacity-50'}`}>English</button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <header className="bg-white shadow-sm p-4 sticky top-0 z-10 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-800 text-lg">{t.welcome}, {user.name.split(' ')[0]}</h2>
            </div>
          </div>
          <button onClick={() => setLang(lang === Language.HE ? Language.EN : Language.HE)} className="text-sm font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-start">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {list.items.filter(i => !i.isChecked).length} items • {list.sharedWith.length > 0 ? t.shared_with + ' ' + list.sharedWith.length : ''}
                    </p>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-xs">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowCreateList(false)} className="px-4 py-2 text-gray-500 font-medium">{t.cancel}</button>
                <button onClick={handleCreateList} className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-medium">{t.create}</button>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="flex items-center justify-between p-4">
          <button onClick={() => setActiveListId(null)} className="p-2 -ml-2 rtl:-mr-2 text-gray-600 hover:bg-gray-100 rounded-full">
            <ArrowLeftIcon />
          </button>
          <h1 className="text-lg font-bold text-gray-800 truncate flex-1 text-center mx-2">{activeList?.name}</h1>
          <div className="flex gap-2">
             <button onClick={handleShare} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-full">
              <ShareIcon />
            </button>
            <button onClick={() => setShowSettings(!showSettings)} className="p-2 text-gray-600 hover:bg-gray-100 rounded-full">
              <CogIcon />
            </button>
          </div>
        </div>
        ...
        {showSettings && (
          <div className="absolute top-full right-4 rtl:right-auto rtl:left-4 bg-white shadow-xl rounded-xl p-2 w-48 border border-gray-100">
            <div className="text-xs font-bold text-gray-400 px-3 py-2 uppercase">{t.settings}</div>
            <button className="w-full text-start px-3 py-2 text-gray-700 hover:bg-gray-50 rounded-lg text-sm flex items-center justify-between">
              {t.sort_groups}
              <span className="text-xs bg-gray-100 px-1 rounded">Drag</span>
            </button>
          </div>
        )}
      </header>
      ...
      <main className="flex-1 p-4 overflow-y-auto pb-32">
        {groupedItems.length === 0 ? (
          ...
          groupedItems.map(({ group, items }) => (
            <div key={group.id} className="mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              {/* Group Header */}
              <div className="bg-gray-50 px-4 py-2 flex items-center justify-between border-b border-gray-100">
                <div className="flex items-center gap-2">
                  <span className="text-xl">{group.icon}</span>
                  <span className="font-bold text-gray-700 text-sm">
                    {(t as any)[group.translationKey] || group.translationKey}
                  </span>
                </div>
              ...
              <div className="divide-y divide-gray-100">
                {items.map(item => (
                  <div 
                    ...
                    className={`p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition-colors ${item.isChecked ? 'bg-gray-50/50' : ''}`}
                  >
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${item.isChecked ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300'}`}>
                      {item.isChecked && <CheckIcon />}
                    </div>
                    <span className={`flex-1 text-base ${item.isChecked ? 'text-gray-400 line-through decoration-emerald-500/40' : 'text-gray-800'}`}>
                      {item.name}
                    </span>
                    {/* Avatar of who added it */}
                    <img 
                      src={item.addedBy === user.id ? user.avatarUrl : OTHER_USER.avatarUrl} 
                      alt="User" 
                      className="w-5 h-5 rounded-full opacity-50"
                      title={item.addedBy === user.id ? "You" : "Other User"}
                    />
                  </div>
                ))}
              </div>
